...
# предсказуемый первый экран: в Chrome на телефоне владельца остаются его собственные вкладки,
# и без этого агент работает поверх них (см. `run`). Адрес можно сменить переменной
# окружения — если поисковик начнёт отдавать капчу или согласие на cookies, достаточно
# поставить здесь другой.
...
class Agent:
    ...
    def run(self, task: str) -> AgentResult:
        ...
        Прогон всегда начинается со стартовой страницы в Chrome: без этой привязки агент стартует
        с того экрана, что был открыт, и спокойно начинает действовать в чужом приложении — на
        живом прогоне он оказался в приложении Google вместо браузера. Обратная сторона той же
        ошибки виднее на телефоне владельца: там в Chrome остаётся открытой его собственная
        вкладка (Google Flights), агент получает её как первый экран и ищет «что такое ллм» в
        форме поиска авиабилетов.
        """
