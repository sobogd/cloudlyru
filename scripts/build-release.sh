...
# iOS: сборка и публикация живут в scripts/build-ios.sh — там своя подпись через App Store
# Connect API и экспорт .ipa. Номер он берёт из pubspec сам, а файл окружения прокидывается,
# чтобы ключи S3 читались из того же места, что у остальных платформ.
#
