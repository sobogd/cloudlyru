...
- SOAP-конверт для AEAT: собранный перенесённым кодом конверт для `FACT-2026-00049`
  **совпал байт в байт** (2790 байт) с тем, что реально ушёл в AEAT и был принят
  (он сохранён в журнале событий фактуры). То есть XML, расчёт НДС, снимки сторон и поля
  цепочки воспроизводятся ровно так, как их приняла налоговая.

Единственное, что нельзя проверить до окна, — расшифровка сертификата и сам сетевой вызов
AEAT: мастер-ключ живёт только на сервере и на рабочую машину не переносится. Если ключ
передан неверно, первая же отправка вернёт ошибку расшифровки, запись цепочки откатится,
фактура останется черновиком — цепочка при этом не пострадает.
