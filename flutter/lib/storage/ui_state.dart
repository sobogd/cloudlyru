...
/// - `chat_model` — идентификатор выбранной модели чата (`AiModel.id`): пишет контроллер чата
///   при выборе модели в шапке и читает при создании, поэтому выбор переживает перезапуск;
...
class UiStateStore {
  ...
  /// Идентификатор выбранной в чате модели (`AiModel.id`) или `null`, если он не сохранён.
  ///
  /// Хранится именно идентификатор, а не индекс в списке моделей: список приходит от LM Studio и
  /// между запусками меняется (модели добавляются и снимаются), поэтому сохранённый индекс
  /// однажды указал бы на чужую модель. Тип проверяем по той же причине, что и в [tab]:
  /// значение приходит с диска, а `as String` на числе уронил бы создание контроллера чата.
  String? get chatModel {
    final v = read()['chat_model'];
    return (v is String && v.isNotEmpty) ? v : null;
  }

  /// Запомнить выбранную модель чата (см. [chatModel]).
  Future<void> setChatModel(String id) => patch({'chat_model': id});
  ...
  /// Режим агента на телефоне; по умолчанию выключен.
  ///
  /// Выключен, а не включён, потому что прогон занимает минуты и телефон при этом один:
  /// оставшись включённым после одного вопроса, режим сделал бы такой же долгой каждую следующую
  /// отправку. Тип проверяем по той же причине, что и в [chatSearch]: с диска приходит что угодно,
  /// а значение не того типа означало бы «агент включён» там, где человек его не включал.
  bool get chatAgent => read()['chat_agent'] == true;

  /// Запоминает режим агента (см. [chatAgent]).
  Future<void> setChatAgent(bool on) => patch({'chat_agent': on});
