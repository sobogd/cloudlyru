...
/// - `chat_style` — идентификатор стиля ответа (`AiStyle.id`): тот же механизм, что у модели.
///   Хранится здесь, а не только в чате, потому что им пользуется новый чат: выбранный однажды
///   «пещерный» стиль продолжается в следующем разговоре, пока человек сам не переключит.
///
...
class UiStateStore {
  ...
  ///
  /// Хранится именно идентификатор, а не индекс в списке моделей: список приходит от xAI и
  ...
  /// Идентификатор последнего выбранного стиля ответа (`AiStyle.id`) или `null`.
  ///
  /// Тип проверяем по той же причине, что и в [chatModel]: значение приходит с диска, а `as
  /// String` на числе уронил бы создание контроллера чата.
  String? get chatStyle {
    final v = read()['chat_style'];
    return (v is String && v.isNotEmpty) ? v : null;
  }

  /// Запомнить стиль ответа для этого и следующих чатов (см. [chatStyle]).
  Future<void> setChatStyle(String id) => patch({'chat_style': id});
