...
///
/// Ключа провайдера здесь нет и быть не должно: к модели ходит сервер своим ключом
/// (`GROK_API_KEY` в его окружении), а приложение — только в наши ручки `/ai/*`. Из-за этого
/// ключ не попадает в сборку (APK лежит на публичной ссылке и извлекается), а причины отказов
/// провайдера видны в логах сервера, а не только сообщением на экране.
library;
...
///
/// Отдельный тип, а не [ApiException] облака, ради одного различия: «на сервере не задан ключ»
/// (503 с кодом `ai_not_configured`) — это не сбой запроса, а состояние сервера, и человеку
...
class AiApiException implements Exception {
  ...
  /// Ключ на сервере не задан — чинится секретом репозитория, а не повтором запроса.
...
class AiApi {
  ...
  /// Состояние раздела: задан ли ключ на сервере и какие модели по нему доступны.
  ...
  /// не задан ключ xAI»). Свой текст оставляем на случай, когда ответа нет вовсе.
