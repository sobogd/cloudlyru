...
/// Зум, с которого клетка рисуется миниатюрой кадра; ниже — простой точкой.
///
/// Миниатюра — это картинка из сети на каждый маркер. На обзорных видах клеток тысячи, и
/// тысячи запросов в одном кадре делают движение карты рваным, а сами миниатюры всё равно
/// неразличимы. Точка рисуется локально и стоит почти ничего, поэтому до этого зума маркер —
/// кружок, а миниатюры включаются только вблизи.
const _thumbZoom = 15.0;
/// Зум, с которого каждая фотография показывается своим маркером, а не клеткой сетки.
///
/// До него точки схлопываются по экранной клетке: на отдалении без этого выходит каша из
/// наложенных маркеров, а в клетке на 17-м зуме и так около 40 метров. С `_exactZoom` клетка
/// не нужна — группируются только кадры с совпадающими координатами (см. `_computeClusters`).
const _exactZoom = 17.0;
/// Область тапа точечного маркера: сама точка мелкая, а палец — нет.
const _dotTap = 26.0;
/// Диаметр точки, под которой один кадр.
const _dotSide = 12.0;
/// Плашка клетки со счётчиком кадров: ширина равна клетке (`_clusterPx`), иначе плашки
/// соседних клеток налезали бы друг на друга и перехватывали тапы.
const _dotCountW = 36.0;
const _dotCountH = 18.0;
...
/// заново на каждое движение карты. Точки в этом смысле дешевле миниатюр (нет картинок из
/// сети), но виджет строится всё равно на каждую — поэтому потолок общий для обоих видов.
...
///
/// Вид маркера зависит от зума: вблизи клетка показывается миниатюрой кадра, на обзорных
/// видах — простой точкой со счётчиком (см. `_thumbZoom` и `_marker`). Склейка точек в клетку
/// тоже зависит от зума: с `_exactZoom` каждая фотография стоит своим маркером на своём месте
/// (см. `_computeClusters`).
///
...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  /// Зум, на котором посчитаны `_clusters`.
  ///
  /// Хранится рядом с кластерами, а не читается из камеры в `build`: от зума зависит вид
  /// маркера (точка или миниатюра), а камера меняется каждый кадр жеста — читай её в `build`,
  /// и перерисовка пошла бы на движение пальца. Здесь снимок зума делается там же, где
  /// пересчитываются кластеры, то есть не чаще троттлинга.
  ...
  ///
  /// С зума `_exactZoom` сетка не используется: клетка — это сами координаты, то есть каждая
  /// фотография получает свой маркер в своём месте (см. `_computeClusters`).
  ///
  ...
  ///
  /// Ключ группы зависит от зума. До `_exactZoom` это номер экранной клетки `_clusterPx`:
  /// рядом стоящие кадры схлопываются в один маркер. С `_exactZoom` ключ — сами координаты,
  /// округлённые до той же точности, что отдаёт сервер (5 знаков, около метра), поэтому у
  /// каждой фотографии свой маркер точно на её месте. Совпадать в таком ключе могут только
  /// кадры с одной и той же геометкой GPS — и это правильно: маркеры в одной точке полностью
  /// накладываются друг на друга, различить и тапнуть их нельзя, а один маркер со счётчиком
  /// открывает их все в просмотрщике.
  List<_Cluster> _computeClusters() {
    ...
    final exact = cam.zoom >= _exactZoom;
    ...
    for (var i = 0; i < _points.length; i++) {
      ...
      // Ключ — либо номер клетки в экранной сетке, либо сами координаты: он и группирует точки.
      final key = exact
          ? '${p.lat.toStringAsFixed(5)}:${p.lon.toStringAsFixed(5)}'
          : '${off.dx ~/ _clusterPx}:${off.dy ~/ _clusterPx}';
      final c = cells[key];
      if (c == null) {
      ...
      body: Stack(children: [
        FlutterMap(
          ...
          children: [
            ...
            MarkerLayer(
              markers: _clusters
                  .map((c) => _marker(c, api, _zoom >= _thumbZoom))
                  .toList(),
            ),
          ],
        ),
  ...
  /// Маркер одной клетки: вблизи — миниатюра кадра, вдали — точка.
  ///
  /// Маркер ставится в среднее место клетки (отсюда деление суммы на счётчик), поэтому у клетки
  /// с несколькими кадрами он может отстоять от реального места съёмки на полклетки —
  /// до `_clusterPx / 2` экранных пикселей. Одиночный кадр в клетке всегда стоит точно на своём
  /// месте: делить его координаты не на что. С `_exactZoom` клеток нет вовсе — среднее считается
  /// только по кадрам с одной геометкой, то есть совпадает с их общей точкой.
  ///
  /// Область тапа задаётся размером маркера, а не точкой внутри: у одиночного кадра это
  /// `_dotTap` при точке в 12 px, у плашки со счётчиком — её собственные габариты.
  Marker _marker(_Cluster c, CloudlyApi api, bool thumbs) {
    final point = LatLng(c.lat / c.n, c.lon / c.n);
    final many = c.n > 1;
    return Marker(
      point: point,
      width: thumbs ? _clusterSide : (many ? _dotCountW : _dotTap),
      height: thumbs ? _clusterSide : (many ? _dotCountH : _dotTap),
      child: GestureDetector(
        onTap: () => _openGroup(c),
        child: thumbs ? _clusterMarker(c, api) : _clusterDot(c),
      ),
    );
  }

  /// Маркер-точка: кружок без картинки — им карта и рисуется на обзорных видах.
  ///
  /// Одиночный кадр — маленький кружок, клетка с несколькими — кружок побольше с их числом.
  /// Число показывается до 999: выше плашка шире клетки, а точный счётчик на таком отдалении
  /// всё равно не читается.
  ///
  /// Побочных эффектов нет.
  Widget _clusterDot(_Cluster c) {
    final many = c.n > 1;
    if (!many) {
      return Center(
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            // Белый ободок: подложка карты светлая, без него синяя точка на ней теряется.
    ...
    return Center(
      child: Container(
        ...
        decoration: BoxDecoration(
          color: C.accent,
          borderRadius: BorderRadius.circular(_dotCountH / 2),
          border: Border.all(color: Colors.white, width: 1.5),
        ),
        child: Text(
          c.n > 999 ? '999+' : '${c.n}',
  ...
  /// Маркер-миниатюра: показывается с зума `_thumbZoom` и выше.
  ///
