...
/// Панель аккаунта: кто вошёл, смена логина и пароля, выход и список входов.
///
/// Всё, что касается входа в аккаунт, собрано в одном месте намеренно: логин и пароль меняются
/// одной формой (доказательство у них общее — текущий пароль), а «выйти» и «завершить вход» —
/// это про одно и то же, про доступ к аккаунту. Раньше вход был показан строкой с кнопкой
/// «Выйти», а сменить логин или пароль из приложения было нечем вовсе: обе ручки существуют
/// на сервере, но кнопки к ним не было.
///
/// Входы показаны списком, и у каждого своя кнопка «Завершить»: одной кнопки «завершить всё»
/// оказалось мало — по ней не видно, что именно закрывается, и она не объясняет, почему
/// оставшиеся строки (app-токены устройств) никуда не делись.
///
