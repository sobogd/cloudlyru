...
/// iOS: папки для синхронизации там не обходятся, а выбираются через системный «Файлы»
/// (см. `sync/device/ios_folders.dart`), и синхронизация идёт, только пока приложение открыто —
/// фоновой работы система не даёт. Панель об этом говорит строкой, а не прячет раздел:
/// остальное (связки, очередь, зеркало) работает как везде.
    ...
    return Panel(
      child: Column(
        ...
        children: [
          ...
          Text(
            granted
                ? 'Работает сама: файл, появившийся или изменившийся на устройстве или в облаке, '
                      'доезжает до другой стороны.'
                : 'Синхронизация не увидит устройство, пока нет доступа ко всем файлам.',
            style: const TextStyle(color: C.fg3, fontSize: 12),
          ),
          // На iOS у синхронизации два своих правила, и оба видны человеку заранее: папки
          // выбираются в «Файлах», а работа идёт, пока приложение открыто (фоновой работы
          // система не даёт). Молчать об этом нельзя — иначе «синхронизация не работает»
          // выглядит как поломка, а не как ограничение системы.
          if (_ios)
            const Padding(
              padding: EdgeInsets.only(top: 6),
              child: Text(
                'Папки выбираются в системном «Файлах», и синхронизация идёт, пока приложение '
                'открыто: в фоне iOS работу не продолжает.',
