...
///
/// Почему именно так, а не «просто открыть ссылку»: приложение ставится APK-ом мимо магазина,
/// а значит обновление — это установка новой сборки поверх. Проверки размера и хеша нужны
/// потому, что APK едет по сети от того же сервера, что и файлы: битая или подменённая сборка
/// дошла бы до установщика, и последствия были бы уже необратимы.
///
...
class _UpdaterPanelState extends ConsumerState<UpdaterPanel> {
  ...
  /// Спрашивает у сервера последнюю сборку (`/app/android`).
  ///
  ...
  ///
  /// Отдельная ветка — `no_release`: APK в бакете есть, а описания сборки рядом с ним нет,
  /// и сервер отвечает на это 404 с кодом `no_release`. Это не сбой проверки (сеть в порядке)
  /// и не битая сборка — обновляться просто не на что, и панель говорит именно это.
  ...
  /// Скачивает сборку, проверяет её и отдаёт системному установщику.
  ///
  ...
  /// совпасть). Только после обеих установщик открывается: APK без проверки ставить нельзя,
  ...
  ///
  /// Побочно: файл во временном каталоге (`cloudly-update.apk` — имя фиксировано, чтобы
  /// повторное скачивание затирало прежний файл), `_progress` на каждом принятом куске,
  /// `_downloading`, `_cancel`, `_error`; в конце — `OpenFilex.open`, то есть системный
  /// установщик.
  Future<void> _update() async {
    ...
    // недокачанный APK иначе лежал бы во временном каталоге до переустановки приложения.
    ...
    File? file;
    // Файл отдан установщику: только тогда его можно оставить на диске (см. `finally`).
    var handedOver = false;
    try {
      final dir = await getTemporaryDirectory();
      file = File('${dir.path}/cloudly-update.apk');
      await _delete(file);
      final dio = Dio();
      ...
      // или подмену файла, и такой APK ставить нельзя. Регистр сравниваем сами: сервер может
