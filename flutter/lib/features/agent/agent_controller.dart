...
/// Клиент моста до pi поверх текущих настроек.
///
/// Провайдером, а не полем состояния: адрес и токены меняются в настройках раздела, и каждый
/// следующий запрос должен уходить уже по новым — поэтому клиент читается заново, а не хранится.
final agentApiProvider = Provider<AgentApi>((ref) => AgentApi(ref.read(settingsProvider)));
