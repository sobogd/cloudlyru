...
/// доли секунды, и качать для них миниатюры — значит занять канал тем, чего человек уже не
/// видит (именно из-за этого видимые клетки оставались серыми).
///
/// Ползунок шкалы в пальце ([scrubbing]) — отдельный запрет: пока человек выбирает месяц, он
/// ещё не знает, куда смотреть, и грузить миниатюры для старого окна бессмысленно. Так «сначала
/// дата, потом картинки» выполняется буквально: окно переставляется, и уже оно просит миниатюры.
...
class _GalleryTileState extends State<GalleryTile> {
  ...
  /// Сколько ждать, если просить пока нельзя (ползунок в пальце, хранилище открывается).
  ...
  void _request() {
    ...
    if (widget.scrubbing.value) {
      // Ползунок в пальце: пока не отпустят, миниатюры не грузим.
      _arm(_retry);
      return;
    }
    _requested = true;
