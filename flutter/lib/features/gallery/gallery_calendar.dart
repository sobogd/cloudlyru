...
/// Где стоит ползунок шкалы: доля таймлайна либо хвост кадров без даты.
///
/// Доля — это время, а не количество кадров: шкала календарно равномерная (см. [GalleryCalendar]),
/// поэтому «середина шкалы» означает середину отрезка съёмки, и ползунок честно показывает,
/// где по времени находится то, что видно в сетке.
class GalleryRailPosition {
  /// Доля от 0 (самое старое) до 1 (самое свежее).
  final double fraction;

  /// Ползунок стоит в зоне кадров без даты — она ниже всей шкалы.
  final bool tail;

  const GalleryRailPosition(this.fraction, {this.tail = false});

  /// Начало таймлайна — положение до первой раскладки.
