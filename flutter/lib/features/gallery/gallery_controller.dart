...
class GalleryController extends ChangeNotifier {
  ...
  /// (`GalleryPages.paused`), а клетки показывают заглушки вместо картинок (см. `GalleryTile`):
  /// декодирование миниатюр — это работа на каждый новый кадр прокрутки, и именно она видна
  /// как «клюёт, когда пытается загрузить». Кадры при этом на месте — библиотека лежит
  /// в памяти целиком (`GalleryPages`), — поэтому «границы подгруженного», на которой раньше
  /// начинался рывок, больше нет: читать на ходу просто нечего.
  ///
  /// По остановке ничего не происходит СРАЗУ: у `GalleryPages` и у плиток пауза в секунду
  /// (свой `_settle` у каждого). Палец замирает между движениями, и работа, начатая по первой
  /// же остановке, приходится на кадр следующего движения — пауза отделяет одно от другого.
  ///
