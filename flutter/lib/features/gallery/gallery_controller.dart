...
class GalleryController extends ChangeNotifier {
  ...
  ///
  /// Пока это идёт, сетка стоит на заглушках известного размера, а по окончании перерисовывается
  /// целиком: строки уже построены по геометрии, и кадры просто встают в свои клетки.
  ///
  ...
  /// (`GalleryPages.paused`): в сетке остаются серые клетки того же размера, и прокрутка идёт
  /// гладко на любой скорости. Кадры при этом на месте — библиотека лежит в памяти целиком
  /// (`GalleryPages`), — поэтому «границы подгруженного», на которой раньше начинался рывок,
  /// больше нет: читать на ходу просто нечего.
  /// По остановке живые строки перерисовываются, а плитки просят миниатюры, то есть грузится
