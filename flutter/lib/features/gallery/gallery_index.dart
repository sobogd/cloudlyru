...
/// арифметикой по блокам, а кадры для построенных строк уже лежат в памяти (`GalleryPages`);
/// виртуальной остаётся отрисовка — построить семьдесят тысяч плиток нельзя ни по памяти,
/// ни по времени кадра (см. `GalleryRowsSliver`).
///
/// ## Чего здесь нет
///
