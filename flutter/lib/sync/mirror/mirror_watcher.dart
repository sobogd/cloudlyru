...
class MirrorWatcher {
  ...
  ///
  /// Считается не от начала прошлого прохода, а от его конца, поэтому на большой библиотеке
  /// проходы и так не накладываются. Пять секунд — компромисс: правка файла должна уехать
  /// почти сразу, но всплеск событий (распаковка архива, пакетное переименование) не должен
  /// превращаться в непрерывный обход. Своя выдержка у наблюдателя всё равно есть
  /// ([debounceMs]), поэтому один проход приходится на серию изменений, а не на каждое.
