...
class MirrorPlan {
  ...
  /// Переименования: файл тот же (совпал ключ файла — том и номер), а путь другой.
...
abstract final class MirrorRules {
  ...
  ///
  /// Сравниваются размер, дата и — когда ключ файла известен с обеих сторон — том и номер
  /// файла в файловой системе. Пара «размер + дата» слепа к подмене содержимого с сохранением
  /// обоих (`cp -p`, `rsync -a`, распаковка архива с датами, восстановление из бэкапа): такой
  /// файл считался бы неизменным и не уехал бы в облако никогда. Ключ файла эту подмену видит.
  ///
  /// Ключ сравнивается только когда совпал том: у съёмного тома номер устройства меняется при
  /// каждом подключении, и «том не тот» означает «сравнить нельзя», а не «файл изменился» —
  /// иначе возвращение карты памяти в телефон стоило бы повторной выгрузки всего её содержимого.
  static bool changed(MirrorRow row, LocalFile file) =>
      row.size != file.size ||
      row.mtime != file.mtime ||
      (row.id.known && file.id.known && row.id.dev == file.id.dev && row.id.ino != file.id.ino);
  ...
  static MirrorPlan plan({
    ...
    // ключу файла). Служебные и скрытые имена сюда не попадают: обход их не показывает — значит
    ...
    final lost = known.values
        ...
        .toList();
    // Ключ файла — единственный признак, по которому переименование отличается от
    // «удалил и залил заново». Ключ без номера (не ответил `stat`) в карту не берём: по нему
    // нельзя утверждать, что это тот же файл. Том в ключе обязателен: без него номер файла
    // с одной карты памяти совпал бы с номером чужого файла на другой
    final byId = <FileId, MirrorRow>{};
    for (final row in lost) {
      if (row.id.known) byId[row.id] = row;
    }
    ...
    for (final file in local) {
      ...
      // Переименование признаём только при совпадении ключа файла (том и номер), размера И даты:
      // одного номера мало — ядро отдаёт освободившийся номер новому файлу, и тогда «удалил A,
      // создал B» выглядело бы переименованием, а облачная запись A перезаписывалась бы
      // содержимым B.
      final moved = file.id.known ? byId[file.id] : null;
      if (moved != null &&
