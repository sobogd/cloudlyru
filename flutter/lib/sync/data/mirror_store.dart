...
///     возвращать содержимое. Пары — это и есть область работы зеркала: проход заводит их
///     из связок человека (см. `SyncLinks`), снятые связки со своими парами уходят, и папки
///     без пары зеркало не читает и не трогает;
///   • ключ: `local_path` (путь связанной папки на телефоне); `cloud_id` — id папки в облаке,
///     `cloud_path` — её путь (показывается человеку как есть);
///   • инвариант: одна строка на связанную папку. Пару снимают вместе со снятием связки
///     ([dropRoot]), а строки `files` при этом остаются — вернуть связку можно без повторной
...
class MirrorStore {
  ...
  /// Какие связки («папка телефона → папка облака») действовали в прошлый проход.
  ///
  /// Связку перенаправили на другую папку облака — по этой записи сверка понимает, что
  /// соответствие папок сменилось: пары и сессии прежней цели снимаются
  /// (см. [MirrorStore.dropDirMapping]), а строки `files` остаются при своих папках.
  ///
  /// Строки, чьих связок больше нет, здесь остаются намеренно: связку сняли и вернули ту же
  /// самую — по этой записи видно, что цель не менялась, и повторной выгрузки не будет.
  ///
  /// Прежний ключ состояния (`mirror_root_id`, id корня `‹Имя устройства› - Файлы`) больше
  /// не читается и не пишется: корни зеркала заводил сервер, а теперь папку в облаке выбирает
  /// человек. Писавшие его сборки ещё могут вернуться откатом обновления, поэтому строка
  /// в базе не удаляется.
  static const String keyLinks = 'links_state';
  ...
  /// Папку сняли со связи: пару убираем, а строки files остаются — вернуть связку можно
  /// без повторной заливки и без удаления в облаке.
  ///
  /// Строка в `dirs` тоже остаётся: она про соответствие папок, а не про связку. Что правки
  /// из журнала не полезут в папку без связки, держат проверки по корням в `MirrorPull`
  /// (`MirrorRules.underRoots`) — корни берутся из этой же таблицы.
  Future<void> dropRoot(String localPath) =>
      _db.delete('roots', where: 'local_path = ?', whereArgs: [localPath]);

  /// Забыть соответствие папок у папки на телефоне и её поддерева.
  ///
  /// Нужно ровно в одном случае: связку перенаправили на другую папку облака. Пары и сессии
  /// незавершённых выгрузок указывают на папки прежней цели, и держать их нельзя: `dirId`
  /// находит пару по пути на телефоне, а две пары на один путь дают то старую папку, то новую
  /// (и тогда файл уедет не туда, куда смотрит связка). Пары поддерева заведёт обход текущего
  /// прохода — по новым папкам.
  ///
  /// Строки `files` остаются намеренно: они описывают записи в конкретной папке облака, и это
  /// знание не ложное — файл правда лежит там. Что выгружать надо в новую папку, решает
  /// сверка по папке облака каждой строки (`MirrorEngine._cloudIdsOfLinks`), а вернут связку
  /// к прежней папке — строки снова окажутся на месте, и выгружать заново не придётся.
  ///
  /// В облаке не удаляется ничего: файлы уедут в новую цель заново, а сервер схлопывает
  /// одинаковое содержимое по sha256, поэтому байты второй раз не передаются.
  ///
  /// Одной транзакцией: наполовину снятое состояние опаснее пустого — движок увидел бы часть
  /// пар снятой, а часть нет.
  Future<void> dropDirMapping(String localPath) => _db.transaction((txn) async {
    final prefix = localPath.endsWith('/') ? localPath : '$localPath/';
    // шаблон LIKE с экранированием: `_` и `%` в имени папки — метасимволы (см. filesUnder)
    final like = '${_likePrefix(prefix)}%';
    await txn.delete(
      'dirs',
      where: "local_path = ? OR local_path LIKE ? ESCAPE '\\'",
      whereArgs: [localPath, like],
    );
    await txn.delete(
      'uploads',
