...
class MirrorStore {
  ...
  /// папке, и держать их нельзя: сверка сочла бы файлы уже выгруженными, а новая папка
  /// осталась бы пустой (см. [MirrorStore.dropLocalState]).
  ///
  ...
  /// Забыть всё, что зеркало знает о папке на телефоне и её поддереве.
  ///
  /// Нужно ровно в одном случае: связку перенаправили на другую папку облака. Строки `files`
  /// описывают записи в прежней папке, и по ним сверка считала бы файлы уже выгруженными —
  /// новая папка осталась бы пустой. Строки `dirs` и незавершённые выгрузки уходят вместе
  /// с ними: и те и другие указывают на папки прежней цели.
  ///
  /// В облаке при этом не удаляется ничего: связку снимают и ставят заново руками, а прежняя
  /// папка остаётся как есть. Файлы уедут в новую цель заново — сервер схлопывает одинаковое
  /// содержимое по sha256, так что байты не передаются второй раз.
  ///
  /// Одной транзакцией: наполовину снятое состояние опаснее пустого — движок увидел бы часть
  /// строк снятой, а часть нет.
  Future<void> dropLocalState(String localPath) => _db.transaction((txn) async {
