...
///
/// Разделом «Файлы» это больше не управляет: там пары «папка устройства ↔ папка облака»
/// задаёт человек связками (см. `SyncLinks`), и папку в облаке приложение не заводит. Список
/// «Файлов» остался только на чтение — из него разово переносятся связки при обновлении
/// приложения ([SyncController] → `_migrateLegacySelection`).
///
/// Пользуются: экран выбора папок «Фото» (`FolderTreeScreen` — отметить и снять), наполнение
/// очереди ([QueueBuilder]) и перенос прежнего выбора в связки.
class Selection {
