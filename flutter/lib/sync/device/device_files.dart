...
/// (`queue/queue_builder.dart`, `queue/queue_refresher.dart`), а через них — наполнение очереди
/// в приложении и в фоне (`background/background_pass.dart`). Обход зеркала — отдельный код
/// (`mirror/mirror_scanner.dart`): у зеркала другие требования к данным.
class DeviceFiles {
  /// [native] — мост к Android: отсюда берутся корни томов. По умолчанию создаётся настоящий
  /// мост; параметр нужен, чтобы передать уже существующий (в фоне его создают один раз и
  /// используют и в зеркале, и в очереди).
  ...
  ///
  /// Просто переадресация в Android — здесь её держат, чтобы вызывающие не знали про мост
  /// и работали только с [DeviceFiles].
