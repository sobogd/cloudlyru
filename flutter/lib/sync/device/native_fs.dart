...
/// | | Android | macOS |
/// |---|---|---|
/// | корни | внутренняя память и съёмные тома из `/storage` | домашняя папка и тома из `/Volumes` |
/// | доступ | «ко всем файлам» проверяется пробой каталога | такого разрешения нет: доступ решает TCC, а его отсутствие видно как нечитаемые папки |
/// | наблюдение | опрос дат папок раз в [pollMs] | `Directory.watch(recursive: true)` на FSEvents: одно наблюдение на всё дерево |
/// | имя устройства | `getprop` (производитель и модель) | `Platform.localHostname` |
///
...
class NativeFs {
  ...
  /// просто не распознаётся и файл уедет заново — это дороже, но не потеря данных.
  ///
  /// Зовётся по одному файлу там, где файл уже выбран (скачивание, разбор переименований);
  /// на массовом обходе — [inodes].
  int inode(String path) {
    final ino = _stat.inode(path);
    ...
    if (!_layoutWarned && _stat.layoutOk == false) {
      _layoutWarned = true;
      debugPrint(
        'cloudly-sync: раскладка struct stat не сошлась — номера файлов недоступны, '
  ...
  List<int> inodes(List<String> paths) => _stat.inodes(paths);

  /// Как зовётся устройство: этим именем подписывается корень зеркала в облаке.
  ///
  /// На Android это «производитель + модель» из `getprop`, на macOS — сетевое имя машины.
