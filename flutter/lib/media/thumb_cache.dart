...
class ThumbCache {
  ...
  ///
  /// Восемь, а не четыре: миниатюра весит ~2 КБ, и время её загрузки определяет не объём, а
  /// задержка круга (замер по логам сервера: ~245 мс на запрос при четырёх загрузках — то есть
  /// упор был в параллелизм, а не в канал). Восемь дают ~1900 запросов в минуту — вдвое
  /// быстрее прогрева библиотеки и без риска упереться в серверный потолок
  /// (`/previews` — 3000 запросов в минуту на адрес, src/media/media.controller.ts).
  ...
  /// в памяти ради работы, которая всё равно идёт по 4 загрузки за раз.
