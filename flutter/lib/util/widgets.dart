...
/// Системный отступ снизу — высота полосы навигации Android на этом экране.
///
/// Нужен спискам и формам, у которых задан свой `padding`. `ListView` прибавляет системный
/// отступ к краям содержимого сам, но только тогда, когда `padding` у него не задан
/// (`ScrollView.buildSlivers`), а у экранов без нижней панели приложения он задан почти везде:
/// в приложении это выглядело так, что последняя строка списка или кнопка формы уезжала под
/// полосу навигации. Экран, у которого снизу своя панель (`NavigationBar` в оболочке),
/// значения не получает: `Scaffold` снимает нижний отступ у тела сам.
///
/// Берём `padding`, а не `viewPadding`: с открытой клавиатурой полоса навигации уже перекрыта
/// клавиатурой, и `viewPadding` прибавил бы её второй раз — контент уехал бы вверх.
double navBarInset(BuildContext context) => MediaQuery.paddingOf(context).bottom;
