...
/// Опубликованная сборка Android-приложения — для обновления по кнопке.
///
/// Отдаётся ручкой `/app/android` без авторизации: приложение может стоять с отозванным токеном,
/// а обновиться ему всё равно нужно.
class AppRelease {
  /// Идентификатор пакета сборки (`ru.cloudly.cloudly_flutter`). В интерфейсе не показывается:
  /// обновление ставится поверх своего же пакета, сравнивать его не с чем.
  final String applicationId;
  /// Номер версии сборки — по нему приложение понимает, что есть обновление, сравнивая со своим
  /// `versionCode` из манифеста.
  ///
  /// `0` означает «версия неизвестна»: так сервер отдаёт APK, у которого нет описания сборки
  /// (`latest.json`), — скачать его можно, но обновлением такая сборка не считается. Именно
  ...
  /// Размер APK в байтах (для подписи в настройках).
  final int size;
  // Хеш APK: по нему проверяется скачанное, чтобы не установить битую сборку.
  final String sha256;
  /// Минимальная версия Android, заявленная сборкой; в интерфейсе не показывается — установку
  /// всё равно решает система.
  final int minSdk;
