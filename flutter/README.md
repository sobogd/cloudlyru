...
удалены. Проверка перед релизом — `flutter analyze` (должен быть чистым; его прогоняет
`scripts/build-release.sh`) и запуск приложения на телефоне.

Релиз собирается и публикуется с этого мака: `./scripts/build-release.sh all` (порядок — в
[`../FLUTTER.md`](../FLUTTER.md), раздел «Релиз»). Скрипт делает подписанный APK, архив
`Cloudly.app` и `.ipa`, кладёт всё в S3, откуда сборки отдают
<https://files.iq-factura.com/apk> и соседние ссылки. В GitHub Actions сборок приложений
больше нет — там остался только деплой сервера.
