...
по запросу). Поиск не отвечает с `ddg_blocked` — значит DuckDuckGo отдал капчу: сервис на маке
сам уходит на паузу 15 минут (капча продлевается каждым новым запросом) и отвечает без свежих
данных, пока она не снимется.
