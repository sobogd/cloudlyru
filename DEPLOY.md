...
1. Сервер решает, нужен ли поиск: по тексту вопроса (свежие данные, деньги, погода, житейские
   «как» и «чем», годы не старше прошлого) или по переключателю в приложении (`авто` / `искать
   всегда` / `не искать`). Перевод, правка текста, счёт и код в поиск не уходят даже в режиме
   «искать всегда»: интернет там не источник фактов.
2. `GET /search?q=&n=` на маке отдаёт выдачу (до восьми результатов), `GET /page?url=&max=`
   читает до четырёх верхних страниц целиком (по 6000 символов). Страница, которая отдала
   меньше 400 символов, считается заглушкой (`short`) — по ней модель отвечать не будет, сервер
   возьмёт следующий результат.
3. Страницы нумеруются, их текст и адреса уходят модели отдельным блоком; модель ссылается на
   источник номером (`[1]`), а не адресом — так она не ошибается в длинных ссылках.
4. Источники сохраняются в `chat_sources` вместе с ответом. Приложение по номеру достаёт точный
   адрес, делает ссылку в тексте и показывает список источников под ответом.

Почему поиск браузерный: сырые HTTP-запросы поисковики режут капчей по отпечатку клиента, а
настоящий Chrome с того же адреса отвечает нормально. Браузер закрывается после 10 минут простоя
(на маке с загруженной моделью память дороже пары секунд запуска), а ответы кэшируются в памяти:
поиск на 10 минут, страницы на 30. Селекторы выдачи и список движков (DuckDuckGo → Startpage →
Bing) лежат в `agents/websearch/browser.py`.

Поиск отвечает ошибкой — смотрим, что в ней:

| Ответ | Что значит |
|---|---|
| `502` и в теле перечислены движки | не ответил ни один: капча у всех, нет сети или браузер не поднялся |
| `short: true` у страницы | сайт отдал заглушку или paywall — модель ответит по выдержке из выдачи |
| пустой список результатов | движок ответил, но разбор не нашёл результатов — почти всегда поменялась разметка выдачи |
