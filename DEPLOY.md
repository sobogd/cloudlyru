...
|---|---|
| `AGENT_START_URL` | страница, с которой начинается каждый прогон (по умолчанию `https://www.google.com/`). Нужна, чтобы первый экран не выбирал Chrome: без адреса он показывает восстановленную вкладку прошлого сеанса |
| `AGENT_SEARCH_URL` | шаблон адреса выдачи, `{query}` подставляется закодированным запросом (по умолчанию `https://www.google.com/search?q={query}`). Поиск идёт по адресу, а не набором текста в поле — см. ниже |
| `AGENT_MIN_PAGES` | сколько разных страниц агент обязан прочитать, прежде чем ему разрешат ответить (по умолчанию `5`). Ответ по сниппетам выдачи агент не принимает |
| `AGENT_LLM_REASONING` | `reasoning_effort` для модели шагов: `none` (по умолчанию), `low`, `medium`, `high`; пусто — параметр не отправляется. На `none` прогон в разы короче: замер дал 24 секунды размышлений на один шаг |
...
Поиск агент делает действием `ACTION: search запрос`, а не набором текста: адрес выдачи собирает
`agent.py` и открывает его напрямую. Через интерфейс искать оказалось нельзя — над полем ввода
Chrome висит список подсказок и прежних запросов, и `type` (который сначала касается элемента)
попадал в строку подсказки: тап уводил браузер на чужой запрос, а набранный текст доставался уже
другой странице. Заодно `type` теперь отказывает, если выбранный элемент — не поле ввода
(`EditText`), а кириллица в запросе работает: в адресе она кодируется, тогда как `input text`
её молча терял.

Читает агент источники, а не выдачу. Выдача — это список ссылок, и ответ по ней выходил
пересказом чужих заголовков, поэтому `done` не принимается, пока не открыто `AGENT_MIN_PAGES`
разных страниц: счёт ведётся по адресу из адресной строки, повторы не считаются, выдача и
домашняя страница поисковика — тоже. Модели в каждом шаге показывается счётчик («прочитано 3 из
5»), а в отчёт для чата уходят и заметки, и список прочитанных адресов — по ним модель чата
может сослаться на источник.
