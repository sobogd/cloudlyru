...
Модель (Gemma 4 E4B в LM Studio) и сервис поиска работают не на сервере, а на домашнем маке
владельца. Сюда они приходят **reverse-SSH туннелем мака** (`jevel.ai/agents/run-dsh-tunnel.sh`,
launchd `com.agent.dsh-reverse-tunnel`), который открывает на VPS два loopback-порта:
...
Наружу они не смотрят вовсе: cloudflared и nginx для чата не нужны, потому что облачный сервер
и конец туннеля — одна и та же машина. Переменные `LLM_BASE_URL`, `LLM_MODEL`, `SEARCH_URL`
задаются в `deploy.yml` (переопределяются одноимёнными секретами репозитория).

Что держит мак в строю, тоже на маке: launchd `com.agent.lmstudio` (сторожит приложение,
сервер на 1234 и загруженную модель) и `com.agent.search` (сервис поиска). Плюс `pmset`
с выставленным `sleep 0` — иначе уснувший мак выглядит как «модель недоступна».

Диагностика, когда чат отвечает «локальная модель недоступна»:

```bash
# на маке: живы ли туннель и агенты
launchctl list | grep -E "dsh-reverse-tunnel|lmstudio|search"
tail -20 /tmp/lmstudio-agent.log /tmp/search-agent.log
# на VPS: слушают ли порты (пусто — туннель не поднялся)
ss -ltn | grep -E "18812|18814"
# на VPS: отвечает ли модель и поиск
