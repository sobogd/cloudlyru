...
1. Сервер решает, нужен ли поиск: по тексту вопроса (свежие данные, деньги, погода, годы не
   старше прошлого) или по переключателю в приложении (`авто` / `искать всегда` / `не искать`).
2. `GET /search?q=&n=` на маке отдаёт выдачу, `GET /page?url=&max=` читает до двух верхних
   страниц целиком (по 6000 символов). Страница, которая отдала меньше 400 символов, считается
   заглушкой (`short`) — по ней модель отвечать не будет, сервер возьмёт следующий результат.
