...
Пароль владельца задаётся при первом старте (на пустой БД первый пользователь создаётся из
`ADMIN_LOGIN`/`ADMIN_PASSWORD`) и меняется ручкой `POST /auth/password` — только веб-сессией,
в приложении такой кнопки нет. App-токены (WebDAV, устройства) отзываются ручкой
