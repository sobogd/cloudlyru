...
> Статус: план M0 выполнен — сервер в проде, клиент — Flutter-приложение (`flutter/`). Документ
> оставлен как история решений M0; как всё устроено сейчас — в `README.md`, `FLUTTER.md`,
> `DEPLOY.md`. Ниже по тексту номера строк, имена файлов и структура каталогов — плановые, а не
> фактические: веб-клиента и старого нативного `android/` в репозитории нет (коммит 407a490).
...
| `view/<sha>-1080.avif` | полноэкранное превью фото (1080, AVIF; у анимированных источников — WebP) | только `trash purge` |
| `view/<sha>-1080.mp4` | полноэкранное превью видео (1080, H.264 по умолчанию; AV1 при `CONVERT_VIDEO_CODEC=av1` и у ассетов, собранных до смены кодека) | только `trash purge` |
| `view/<sha>-p<N>-1080.webp` | превью страницы PDF (1080, WebP; N с единицы), число страниц — в `Asset.pageCount` | только `trash purge` |
| `view/<sha>.avif`, `view/<sha>-2048.avif`, `view/<sha>-2048.webp`, `view/<sha>.mp4`, `view/<sha>-720.mp4` | **устаревшие**: мастер-версии и превью прежних размеров, больше не создаются; старое превью фото 2048 продолжает отдаваться как легаси-фолбэк, пока ассет не пересобран | только `trash purge` |
...
`Album` (виртуальные коллекции: из файлов/папок), `AlbumItem`, `MediaMeta` (EXIF: captured_at, gps…), `LocationSample` (история «где я был»), `StorageTemplate`. Правило: медиа с EXIF попадает в таймлайн автоматически, файлы без метаданных — только в дерево.
...
`.env.example` (в репо, без значений — актуальный шаблон именно там; здесь исходный список M0):
```
NODE_ENV=production
PORT=8305
...
Ключа-секрета сессии нет: cookie — 32 случайных байта, в БД лежит их sha256, подписывать
нечего (`SESSION_SECRET` стоял в этом плане, но код не читал его ни разу — переменная удалена).
Полный список переменных, которые читает приложение, — `src/config/env.ts` плюс лимиты
распаковки в `src/unzip/unzip.limits.ts`.

Правила: `.env` в `.gitignore`; на сервере `.env` chmod 600; значения не появляются в логах/чате.
Логин и пароль владельца задаются при первом старте (на пустой БД первый пользователь создаётся
из `ADMIN_LOGIN`/`ADMIN_PASSWORD`) и меняются ручкой `POST /auth/credentials` — только
веб-сессией, из настроек приложения (панель «Аккаунт»). Секреты репозитория на живой базе
логин и пароль не меняют: `ADMIN_*` читаются ровно один раз, при создании владельца.
Прочие входы гасит `POST /auth/sessions/revoke-others` (тоже из настроек приложения).
App-токены (WebDAV, устройства) отзываются ручкой `DELETE /auth/tokens/:id`.
...
## 16. Что НЕ входит в M0 (роадмап: что из этого сделано)

- M1: WebDAV (Finder/Mac), корзина-UI, presigned-загрузка. — сделано всё три.
- M2: фото-таймлайн по EXIF, альбомы. Карта и авто-определение поездок — снято: от них
  отказались, из продукта они убраны. — лента «Медиа», карта и просмотрщик сделаны, поездок нет,
  альбомы остались ручками API без интерфейса.
- M3: Android-клиент (автовыгрузка + share-диалог), затем iOS (шеринг в оригинале + автовыгрузка).
  — Android-клиент на Flutter в проде; share-диалог (приём файла из чужого приложения) убран
  вместе со старым нативным клиентом; iOS-сборки в проекте нет.
- M4: история местоположений («где я был», 5–10 мин), дом. HDD-бэкап, миграция из Google.
  — сделана только миграция из Google (`scripts/backfill-*.mjs`, `scripts/takeout-*.mjs`);
  истории местоположений (таблицы `LocationSample` в схеме нет) и домашнего бэкапа нет.

Лента «Медиа» — не вся зона «Фото», а ровно то, у чего есть строка `MediaMeta` (её заводит
разбор метаданных для любого `image/*` и `video/*`): PDF, архивы и прочее в ленту не попадают
вовсе, BMP/JXL попадают без превью (`previewState = 'impossible'`) и видны только в «Файлах».
