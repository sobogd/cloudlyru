...
|---|---|---|
| `view/<sha>.avif` | фото-мастер (AVIF q85, EXIF+ICC внутри) | только `trash purge` |
| `view/<sha>-512.webp` | грид для ленты | только `trash purge` |
| `view/<sha>-2048.webp` | «полный экран» фото (кап 2048 px) | только `trash purge` |
| `view/<sha>.mp4` | видео-мастер (AV1) | только `trash purge` |
| `view/<sha>-720.mp4` | 720p-превью (именно его играет UI) | только `trash purge` |
| `view/<sha>-poster.webp` | постер видео (кадр ~1 с) | только `trash purge` |
