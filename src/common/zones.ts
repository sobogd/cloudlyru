 ...
 *
 * Про легаси-папку «Телефон»: сервер её больше не заводит и от удаления не защищает, но
 * существующая продолжает работать — её id по-прежнему отдаётся в `GET /auth/me`
 * (`phoneFolderId`, см. AuthService.phoneRootIdOrNull). Константы с её именем здесь нет
 * намеренно: имя нигде не используется в коде, а комментарий-справка живёт там, где папка
 * читается.
 */
