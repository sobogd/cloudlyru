...
const envSchema = z.object({
  ...
  //
  // Модель работает не на сервере, а на домашнем маке (llama.cpp), и приходит сюда через
  ...
  // Ключ к серверу модели: нужен только если он требует ключ (llama-server с --api-key). Обычно пусто.
  LLM_API_KEY: z.string().default(''),
  // Модель по умолчанию для новых чатов. Идентификатор — тот, что отдаёт сервер модели
  ...
  //
  // Параметр не из стандарта OpenAI: его понимал LM Studio, а llama.cpp игнорирует. Другие способы
  ...
  // Дополнительные поля для тела запроса к модели, JSON-объект строкой. Пусто — не отправляются.
  //
  // Нужны потому, что выключение «размышлений» у разных серверов делается по-разному: LM Studio
  // (сносён с мака)
  // понимал `reasoning_effort`, а llama.cpp (на нём теперь считает мак) — `chat_template_kwargs`.
  // Для Qwen это `{"enable_thinking": false}`, и без него модель уходит в размышления целиком
  // и текста не отдаёт вовсе (замер: 300 из 300 токенов — reasoning, пустой ответ).
  LLM_TEMPLATE_KWARGS: z.string().default(''),
  // Агент на телефоне: сервис на маке (agents/phone-agent/), тоже через туннель. Он открывает
  // сайты в настоящем Chrome телефона (ADB + Chrome DevTools): ищет в Google, ищет ВНУТРИ
  // сайта через его поисковую строку (amazon.es, reddit.com) и читает страницы. Именно это
  // делает возможным «найди на реддите» как поиск на Reddit, а не как google-запрос со словом
  // «реддит». Пусто — агент недоступен, и раздел честно отвечает своими знаниями.
  AGENT_URL: z.string().url().default('http://127.0.0.1:18816'),
  // Потолок шагов агента: сколько раз он может сходить в телефон за один вопрос. Каждый шаг —
  // это 5-15 секунд на телефоне плюс шаг модели, поэтому потолок здесь не про экономию, а про
  // предсказуемость: без него разговорчивая модель может ходить по сайтам минутами.
  AGENT_MAX_STEPS: z.coerce.number().int().min(1).max(12).default(6),
  // Поиск и чтение страниц для раздела «Чат»: сервис на маке (agents/websearch/), тоже через
  // туннель. Оставлен как запасной путь и для ручных проверок; в решении о поиске больше не
  // участвует — всё делает агент на телефоне. Он и ищет (браузером, потому что HTTP-клиент с серверного адреса получает капчу),
  // и читает выбранные страницы целиком (`GET /page`) — из этого текста модель и берёт факты.
  // Пусто — раздел работает, но отвечает без свежих данных.
