...
const envSchema = z.object({
  ...
  // Ключ xAI (Grok) для раздела «Чат». Живёт только на сервере: запросы к модели делают
  // серверные ручки (/ai/*), а приложение ходит в них — ключ не попадает ни в сборку, ни
  // в логи (в лог уходит только ответ провайдера). Пусто — раздел отвечает «не настроен»,
