...
const envSchema = z.object({
  ...
  // Мост до харнесса pi на маке (`agents/pi-bridge/`) для раздела «Проекты»: агент работает в
  // папке проекта — читает и правит файлы, запускает команды. Приходит сюда reverse-SSH
  // туннелем мака, поэтому адрес — loopback самого VPS: снаружи порт не смотрит
  // никто, и ни TLS, ни ключей на этом плече нет. Пусто — раздел отвечает «мост недоступен».
  PI_BRIDGE_URL: z.string().url().default('http://127.0.0.1:18820'),
  // Панель управления домашним маком (`mac-status-server.py`) для раздела «Mac»: сервер
  // проксирует ручки `/mac/*` к ней. Ходит тем же reverse-SSH туннелем мака (порт 18810),
  // поэтому адрес — loopback VPS: снаружи порт не смотрит никто, ни TLS, ни ключей на этом
  // плече нет. Пусто — раздел отвечает «мак недоступен».
  MAC_STATUS_URL: z.string().url().default('http://127.0.0.1:18810'),
  // Сервис-токен для ручек панели Mac (`X-Mac-Token`). Значение знают только бэкенд и панель
  // (в `~/work/.env` на маке); приложение его не видит. Пусто — панель работает без проверки.
  MAC_SERVICE_TOKEN: z.string().default(''),
  // Локальный whisper.cpp на маке для голосового ввода в разделе «Проекты»: приложение
  // записывает речь, сервер отдаёт запись сюда и возвращает распознанный текст. Приходит тем
  // же reverse-SSH туннелем мака, что и мост pi (порт 18818), поэтому адрес — loopback VPS:
  // снаружи порт не смотрит никто, ни TLS, ни ключей на этом плече нет. Пусто — голосовой
  // ввод отключён, ручка отвечает отказом.
  STT_URL: z.string().default('http://127.0.0.1:18818'),
  // Язык распознавания для whisper (`ru`, `en`, ...). Пусто — `auto` (автоопределение):
  // в русской речи бывают английские вставки, и жёсткий `ru` заставляет модель озвучивать их
  // по-русски. Именно `auto`, а не «не слать поле»: у whisper-server дефолт — `en`, и без языка
  // русская речь возвращается английским пересказом (см. ProjectsService.transcribe).
  STT_LANGUAGE: z.string().default(''),
  // ===== Локальная модель (перевод писем) =====
  // Модель живёт не на сервере, а на домашнем маке (llama.cpp), и приходит сюда тем же
  // reverse-SSH туннелем, что и мост pi: на VPS открыт только loopback-порт 18812. Запросы
  // делает сервер, а не приложение: в сборку не вшивается ни адрес, ни ключ, а причина отказа
  // (мак спит, туннель отвалился) остаётся в логах pm2.
  //
  // Адрес — не секрет и не настройка вкуса: это тот же порт, что в `run-tunnel.sh` на маке
  // (пункт «llama.cpp, local LLM for CloudlyRu»). Менять его нужно вместе с туннелем, иначе
  // перевод отвечает «локальная модель недоступна».
  LLM_BASE_URL: z.string().url().default('http://127.0.0.1:18812'),
  // Ключ к движку: нужен только если он умеет требовать авторизацию. У llama.cpp обычно пусто.
  LLM_API_KEY: z.string().default(''),
  // Идентификатор модели в том виде, в каком его отдаёт движок (`GET /v1/models`), а не имя
  // файла на диске: смена кванта или каталога модели меняет его, поэтому значение живёт в
  // окружении. Дефолт — честное имя того, что стоит на маке сейчас (`qwen/qwen3.8-27b` из
  // `--alias` в `agents/mac/run-llm.sh`); имена прежних моделей (`qwen/qwen3.5-9b`,
  // `qwen/qwen3.5-4b`) остались там же алиасами, поэтому старые конфиги и уже созданные чаты
  // продолжают работать. У llama.cpp с одним загруженным файлом несовпадение безвредно — он
  // отвечает тем, что загружено.
  LLM_MODEL: z.string().default('qwen/qwen3.8-27b'),
  // Окно контекста ОДНОГО слота движка, токены. Обязано совпадать с `-c` в запуске llama.cpp
  // (там на четыре слота делится общее окно, а один запрос занимает один слот): от этого числа
  // выводятся потолок входа и размер куска перевода (`src/llm/llm-limits.ts`). Превышение окна
  // движок отвергает кодом 400, поэтому расхождение здесь — это отказы, а не тихое урезание.
  LLM_CTX: z.coerce.number().int().positive().default(32768),
  // `reasoning_effort`: none | low | medium | high. Пусто — параметр не отправляется.
  //
  // `none` — не украшение: у Qwen с включённым «размышлением» на короткий ответ уходит весь
  // выданный бюджет токенов, а текста нет вовсе (замер iq translate: 300 из 300 токенов в
  // размышления). Поэтому модели на маке запущены с выключенным thinking, а тот же флаг уходит
  // и в запросе — поведение не зависит от того, как поднят сервер.
  LLM_REASONING: z.string().default('none'),
  // Второй способ выключить размышления — через шаблон чата (`chat_template_kwargs`). Отдельная
  // переменная нужна потому, что движки принимают это поле по-разному; битое значение молча
  // игнорируется, чтобы опечатка в окружении не ломала перевод.
  LLM_TEMPLATE_KWARGS: z.string().default('{"enable_thinking":false}'),
  // Потолок на один ответ целиком, мс. Щедрый намеренно: холодная модель сначала загружается,
  // а длинное письмо переводится несколькими вызовами подряд.
