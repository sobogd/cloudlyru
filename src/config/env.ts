...
const envSchema = z.object({
  ...
  // Карантин между «сохранено у нас» и «удалено с сервера», часы. По умолчанию ноль: копию
  // убираем сразу, потому что перед удалением письмо проверяется поимённо — есть ли байты у
  // нас и лежит ли по нашим координатам на сервере именно оно. Карантин остаётся как
  // необязательная выдержка «пусть полежит» для тех, кому так спокойнее.
  MAIL_PURGE_QUARANTINE_HOURS: z.coerce.number().int().nonnegative().default(0),
  ...
  // Как часто проходить по расписанию, секунды (час по умолчанию). Таймер заводится только
  // при включённом автоудалении: без него расписание не нужно вовсе.
  MAIL_PURGE_INTERVAL_SEC: z.coerce.number().int().positive().default(3600),
  ...
  // Девяносто, а не половина: в маленьком ящике (iCloud — сотня с небольшим писем) под
  // удаление идёт почти всё по замыслу, и половина рубила прогон каждый раз, то есть ящик
  // не очищался вовсе. Настоящую ошибку в отборе видно и по девяноста: доля упирается в сто.
  MAIL_PURGE_MAX_SHARE: z.coerce.number().positive().max(100).default(90),
  // Адреса, письма от которых не удаляем никогда. По умолчанию список пуст: письма
  // провайдеров (коды входа, оповещения, восстановление доступа) у нас уже сохранены целиком,
  // и читать их можно в нашем интерфейсе, так что отдельная защита им не нужна. Настройка
  // осталась для того, кому спокойнее держать такие письма ещё и у провайдера.
  MAIL_PURGE_PROTECT_SENDERS: z.string().default(''),
