...
const envSchema = z.object({
  ...
  LLM_MODEL: z.string().default('google/gemma-4-e4b'),
  // `reasoning_effort` для запроса: none | low | medium | high. Пусто — параметр не отправляется.
  //
  // Для Gemma 4 значение `none` не украшение: без него модель тратит на «размышления» около
  // 90% сгенерированных токенов (замер: 443 из 502), а человек всё это время ждёт первого
  // видимого слова — на маке это десятки секунд. Параметр не из стандарта OpenAI, его понимает
  // LM Studio; другой сервер может его отвергнуть, и тогда достаточно очистить переменную.
