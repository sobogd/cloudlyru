 ...
 *
 * Число — про физические пиксели, а не логические. Сетка показывает четыре кадра в ряд, то
 * есть клетка на экране 360 dp — это 88 dp, а на экране с DPR 3 — 264 физических пикселя.
 * 256 px покрывают это почти один в один (апскейла практически нет), а запас на DPR выше
 * среднего даёт лёгкое растяжение вместо мыла.
 *
 * Цена — вес: AVIF q60 при 256 px это ~10 КиБ на кадр против 2.4 КиБ при 100 px, то есть вся
 * библиотека в 51 700 кадров занимает 515 МиБ данных (627 МиБ на телефоне с округлением до
 * блока 4 КиБ). При 100 px и шести колонках было 119 МиБ, но клетки выглядели мелкими.
 *
