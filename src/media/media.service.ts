...
const IMAGE_MIMES = ['image/jpeg', 'image/heic', 'image/heif', 'image/png', 'image/webp', 'image/tiff', 'image/avif'];
...
export class MediaService {
  ...
  private static gapMs = 36 * 60 * 60 * 1000; // >36ч между кадрами = новая поездка
  ...
  /** Best-effort: читает объект из S3, парсит EXIF и сохраняет MediaMeta. */
  async maybeCapture(assetId: string, sha256: string, size: number, mime: string): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`EXIF parse skip: ${(e as Error).message}`);
    }
  }
  ...
  async timeline(limit = 300, before?: string): Promise<TimelineItem[]> {
    // TODO: вернуть строгую типизацию select'а, когда Prisma-клиент перестанет ломать
    // вывод для вложенного optional-relation фильтра
    const rows = (await this.prisma.fileEntry.findMany({
      where: {
        deletedAt: null,
        asset: {
          media: before ? { capturedAt: { lt: new Date(before) } } : { isNot: null },
        },
      },
  ...
  // ============ Поездки (по координатам + близким датам) ============
