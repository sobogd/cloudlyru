 ...
 * это 150 px, и квадрат 50×50 растягивался втрое — сетка выглядела мыльной.
 *
 * 100, а не больше, из-за того, как эти превью ложатся в файловый кэш телефона: миниатюра
 * хранится отдельным файлом, а файл короче блока файловой системы (4 КиБ) всё равно занимает
 * блок. WebP q78 в 100×100 весит ~2.7 КиБ — всё ещё внутри блока, поэтому кэш всей библиотеки
 * (десятки тысяч кадров) остаётся прежним по месту; 160 px уже переваливают за блок, и место
 * удваивается (замер на реальной библиотеке — в отчёте, `REVIEW.md`). На DPR 3 остаётся
 * полуторный апскейл — компромисс между мылом 50×50 и весом 160 px.
 *
 * Ключ в S3 остался историческим `-512.webp` (менять его нельзя — сломается кэш и старые
 * клиенты): под ним лежат превью прежних версий пайплайна — 512 px и 50×50. Они отдаются как
 * есть, без пересборки: чтобы перевести уже собранную библиотеку на актуальный размер, нужен
 * `scripts/rebuild-grid.mjs` (пересборка из оригинала по фактической ширине объекта) — сам
 * размер ничего не пересобирает.
 */
