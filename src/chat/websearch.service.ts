 ...
 *
 * Четыре, а не две: на «как очистить ковер от запаха собаки» первые две страницы оказались
 * общими советами, а конкретные средства и пропорции нашлись дальше по выдаче. По контексту это
 * 24 000 символов — примерно 8 000 токенов из 32 768, то есть место есть, а время чтения
 * четырёх страниц (по 2-3 секунды) того стоит.
 */
const MAX_PAGES = 4;
...
export class WebSearchService {
  ...
  private async call<T>(path: string, signal: AbortSignal, timeoutMs: number): Promise<T> {
    ...
    try {
      ...
      throw new WebSearchError('сервис поиска не ответил (мак спит или туннель отключился)');
    } finally {
      clearTimeout(timer);
