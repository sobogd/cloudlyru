...
/**
 * Имя части письма, пригодное для дерева: без разделителей пути, управляющих и невидимых
 * символов, без символов, запрещённых в Windows, без ведущих точек и пробелов, с запасным
 * именем вместо пустого. Расширение сохраняем — по нему клиент выбирает приложение для открытия.
 */
export function safeAttachmentName(raw: string | undefined | null): string {
  const cleaned = String(raw ?? '')
    // Управляющие символы (C0 и C1), невидимые и bidi-символы приходят из заголовков соседних
    // писем: Content-Disposition в письме — недоверенный ввод, а имя уезжает в S3-ключ, WebDAV
    // и файловую систему телефона. Bidi-переопределение разворачивает хвост имени
    // («счёт‮fdp.exe» показывается как «счётexe.pdf»), невидимые — склеивают слова.
    .replace(/[\u0000-\u001f\u007f-\u009f]/g, ' ')
    .replace(/[\u200b-\u200f\u202a-\u202e\u2066-\u2069\ufeff]/g, '')
    .replace(/[/\\]/g, '-')
    // Символы, запрещённые в именах Windows: `a:b.txt` там превращается в alternate data
    // stream (файл «a» с потоком «b.txt»), остальные просто не создаются.
    .replace(/[:*?"<>|]/g, '-')
    .replace(/^[.\s]+/, '')
    // Хвостовые точки и пробелы Windows отбрасывает сам — имя в облаке и на телефоне разошлось бы.
    .replace(/[.\s]+$/, '')
    .replace(/\s+/g, ' ')
    .trim();
