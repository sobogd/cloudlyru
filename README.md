...
3. Запустить **три** workflow на этот номер — Android, macOS, iOS:
   `gh workflow run android.yml`, `gh workflow run macos.yml`, `gh workflow run ios.yml`
   (`--repo sobogd/cloudlyru`). Номер сборки общий для платформ, поэтому пропустить одну
   нельзя: опубликоваться позже с тем же номером она уже не сможет, а поднять номер второй
   раз нельзя, пока предыдущая платформа выпущена.
4. GitHub Actions собирают и публикуют сборки: APK — в `/apk`, `Cloudly.app` — в `/macos`,
   `.ipa` — в `/ios` (установка по воздуху — `/ios/install`). Тестов в проекте нет: перед
   выпуском прогоняется `flutter analyze`, а работу приложения проверяет владелец сам.
