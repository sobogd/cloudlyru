...
Сборка и публикация — на своём маке, номер версии поднимает человек: по нему телефон
и понимает, что вышло обновление.

1. Поднять `versionCode` (и `versionName`) в `flutter/pubspec.yaml` (`version: 1.0.0+45`).
2. Push в `main`, если менялся `flutter/**`.
3. Собрать **три** платформы на этот номер: `./scripts/build-release.sh all`. Номер сборки
   общий для платформ, поэтому пропустить одну нельзя: опубликоваться позже с тем же номером
   она уже не сможет, а поднять номер второй раз нельзя, пока предыдущая платформа выпущена.
4. Скрипт собирает и публикует сборки: APK — в `/apk`, `Cloudly.app` — в `/macos`, `.ipa` —
   в `/ios` (установка по воздуху — `/ios/install`). Тестов в проекте нет: перед выпуском
   прогоняется `flutter analyze`, а работу приложения проверяет владелец сам.

Если номер не поднят, публикация падает с понятным сообщением: сборка с тем же номером не
появится в приложении как обновление, а тихо подменять файл по ссылке — значит оставить
телефон на старой версии. Пересобрать ту же версию для проверки можно с `--force`
(`./scripts/build-release.sh android --force`) — кнопкой такое обновление не увидит никто.

Раньше сборка шла в GitHub Actions; от неё отказались ради скорости (локальный инкремент
вместо холодного раннера), в Actions остался только деплой сервера.
